...
mkdir('Automated_simulations');
mkdir('Automated_data');
...
model.param.set('time_step', strcat(num2str(time_step), '[s]'), 'time_to_impact/30');
...
sol1.feature('t1').feature('st1').set('stopcondarr', {'1/timestep > 1e10' 'comp1.minop1(root.z) < 1e-7 [m]'});
...
for i=1:n_times
    writematrix(pres_data{1}{i}.d(idx),strcat(data_folder,'\pressure',num2str(i),'.txt'),'Delimiter','tab')
end
...
%gifs path
data_path = strcat("\Automated_data\", name, '_ms')
...
figure(1)
for i=1:fix(n_times/7):n_times
    subplot(2,1,1)
    coor = tip_data{1}{i}.p(idx);
    tip2 = tip_data{1}{i}.d(idx);
    r_min(i) = coor(find(tip2==min(tip2)));
    plot(coor,tip2, 'DisplayName',num2str(times(i)))
    hold on
    subplot(2,1,2)
    plot(coor,pres_data{1}{i}.d(idx))
    hold on
end
subplot(2,1,1)
set(gca, 'YScale', 'log')
