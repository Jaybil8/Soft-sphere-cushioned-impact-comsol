...
%vel_list_soft = linspace(0.05, 5.4, 30);
...
%}
    switch stiffness
        case 'Soft'
            ...
            impactor_density = 1140;
            impactor_E = 250*10^3;
        case 'Hard'
            impactor_density = 1140;
            impactor_E = 1100*10^3;
        otherwise
            error('Invalid value of the input. Run the program again')
    end
    G = impactor_E/(2*(1+poisson));
    c_s = sqrt(G/impactor_density);

    model_name = strcat('EHD_ball_impact_', stiffness, velocity, 'ms.mph');
    %setting initial height depending on starting conditions
    stk = radius*impactor_density*V/fluid_mu;
    phi= stk^(1/3)*V/c_s;
    if phi<1     %elastic
        h0 = 20*radius^(3/5)*(V*fluid_mu/G)^(2/5);
        pscale = (G^4*V*fluid_mu/radius)^(1/5);
        disp("Elastic")
    else            %inertial
        h0 = 20*radius*stk^(-2/3);
        pscale = impactor_density*V^2*stk^1/3;
        disp("Inertial")
    end
    disp(h0)
...
mkdir('Automated_simulations_regimes');
mkdir('Automated_data_regimes');
model.modelPath(strcat(path,'\Automated_simulatons_regimes'));
name = strcat(stiffness, "_", strrep(velocity,'.','_'));

model.param.set('fluid_density', '1.2 [kg/m^3]', 'density of the fluid');
model.param.set('ball_density', strcat(num2str(impactor_density), '[kg/m^3]'), 'density of the ball');
...
model.param.set('ball_E', strcat(num2str(impactor_E), '[Pa]'), 'young modulus of the ball');
...
model.param.set('h0', strcat(num2str(h0),'[m]'), 'initial height at the start of the smulation (check 10 times is good enough)');
...
model.param.set('time_step', 'time_to_impact/20', 'time_to_impact/10');
model.param.set('shear_modulus', 'ball_E/(2*(1+ball_nu))');
model.param.set('shear_wave_speed', 'sqrt(shear_modulus/ball_density)');
model.param.set('ratio_impact_wave', 'velocity/shear_wave_speed');
model.param.set('Phi', 'ratio_impact_wave/delta');
model.param.set('pressure_scale', pscale); %pressure scale from the scaling
...
study1.feature('time').set('tlist', 'range(0,total_time/200,total_time)');
...
% Save file
mphsave(model_name, strcat("\Automated_simulations_regimes\",name, "_ms.mph"))
...
data_folder = strcat(path,'\Automated_data_regimes\',name);
...
%gifs path
data_path = strcat("\Automated_data_regimes\", name, '_ms');
