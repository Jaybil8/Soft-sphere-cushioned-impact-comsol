...
%max_interval = 0; %.5;
min_interval = -.5; %variations of plus or minus 50%
max_interval = .5;
n_param = 6;
...
if poisson == .47
    poisson_value = 'updated2_phi_poisson_047_';
else
    poisson_value = '';
end
...
for i=1:n_exp
    ...
    E_solid = 675e+4*(1+variaton(6,i));
    shear_modulus = E_solid/(2*(1+poisson));
    cs = (shear_modulus/rho_solid)^.5;
    %p_wave_modulus = E_solid*(1-poisson)/((1+poisson)*(1-2*poisson));
    vel(i) = (6*phi(i)*shear_modulus^.5*(mu_air/radius)^(1/3)/rho_solid^(5/6))^(3/4);
    %ell(i) = 6*radius^(2/3)*(mu_air/(rho_solid*vel(i)))^(1/3);
    %ell elastic
    ell(i) = (pi*mu_air*radius^3*vel(i)/(2*E_solid/(1-poisson^2)))^(1/4);
    %ell elastic John
    %ell(i) = (pi*mu_air*radius^3*vel(i)/(2*E_solid/(1-poisson^2)))^(1/4);
    h0(i) = 36*radius^(1/3)*(mu_air/(rho_solid*vel(i)))^(2/3);
end
...
color_idx2 = color_len;
    for i= 1:n_exp
        ...
        T_impact = h0/vel(i);
        data_folder = strcat(poisson_value,'phi',  strrep(num2str(phi(i)),'.','_'), 'highE_perturbed');
        %data_folder = strcat(poisson_value,'phi',  strrep(num2str(phi(i)),'.','_'), 'highE');
        cd(data_folder)
        ...
        exportgraphics(gcf,strcat('impact', poisson_value,'phi',num2str(phi(i)) ,'highE_perturbed.png'),'Resolution',300, 'BackgroundColor','none') 
        %saveas(gcf, )
        close
        %}
        r0(i) = r0(i)/ell(i);
...
cd '\Users\jbilotto\Documents\EPFL E3 program 2021\research project\Comsol_simulations\Automated_figures_doe\'
exportgraphics(gcf, strcat('radius_evolution_', poisson_value ,'phi_',num2str(phi(i)) ,'highE_perturbed.png'), 'Resolution',300, 'BackgroundColor','none')
close
%}
...
writematrix([phi;round(r0,2)'], strcat('tweaked',poisson_value,'highE_r0_phi.txt'))
xlabel('$\phi$','Interpreter','latex')
ylabel('$r_{contact}/\mathcal{L} $','Interpreter','latex')
title(strcat('Poisson', num2str(poisson)))
legend('DoE', 'Location','best')
fontsize(fig, 16, 'points')
cd '\Users\jbilotto\Documents\EPFL E3 program 2021\research project\Comsol_simulations\Automated_figures_doe'
%exportgraphics(gcf, strcat('tweaked_trapped_radius_scaled_phi_ell_doe',poisson_value,'highE_perturbed.png'), 'Resolution',300, 'BackgroundColor','none')
exportgraphics(gcf, strcat('tweaked_trapped_radius_scaled_phi_ell_doe',poisson_value,'highE_perturbed.png'), 'Resolution',300, 'BackgroundColor','none')
%}
end
