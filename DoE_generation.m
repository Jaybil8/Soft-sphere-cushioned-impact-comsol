...
for i=1:n_exp
    ...
    vel(i) = (phi(i)*(mu_air/radius)^(1/3)*p_wave_modulus^.5*rho_solid^(-5/6))^(3/4);
    ell(i) = radius^(2/3)*(mu_air/(rho_solid*vel(i)))^(1/3);
    h0(i) = radius^(1/3)*(mu_air/(rho_solid*vel(i)))^(2/3);
...
name = strcat('poisson_', strrep(num2str(poisson),'.','') ,'_phi',strrep(num2str(phi),'.','_'));
