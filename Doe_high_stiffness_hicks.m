...
for i=1:n_exp
    ...
    vel(i) = (6*phi(i)*cs)^(3/4)*(mu_air/(rho_solid*radius))^(1/4);
    ell(i) = 6*radius^(2/3)*(mu_air/(rho_solid*vel(i)))^(1/3);
    h0(i) = 36*radius^(1/3)*(mu_air/(rho_solid*vel(i)))^(2/3);
    phi(i) =(rho_solid*radius/mu_air)^(1/3)*vel(i)^(4/3)/(6*cs);
...
model_name = strcat('EHD_ball_impact_poisson_',strrep(num2str(poisson),'.','') ,'_phi', num2str(phi), 'highE_perturbed_.mph');
...
name = strcat('updated2_phi_poisson_', strrep(num2str(poisson),'.','') ,'_phi',strrep(num2str(phi),'.','_'), 'highE_perturbed');
