...
if poisson == .1
    poisson_value = 'updated2_phi_poisson_01_';
else
    poisson_value = '';
end
...
for i=1:n_exp
    ...
    %elastic
    %ell(i) = (radius^4*mu_air*vel(i)/shear_modulus/(1-poisson))^(1/5);
    %inertial
    %ell(i) = radius^(4/5)*(mu_air/(rho_solid*vel(i)))^(1/5);
    %bulk
    bulk_modulus = E_solid/(3*(1-2*poisson));
    ell(i) = (radius^5*mu_air*vel(i)/bulk_modulus)^(1/100);
...
cd '\Users\jbilotto\Documents\EPFL E3 program 2021\research project\Comsol_simulations\Automated_figures_doe'
exportgraphics(gcf, strcat('updated_trapped_radius_scaled_phi_ell_doe',poisson_value,'.png'), 'Resolution',300, 'BackgroundColor','none')
%}
end
